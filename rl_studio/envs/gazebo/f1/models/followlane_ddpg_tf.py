...
class FollowLaneDDPGF1GazeboTF(F1Env):
    ...
    def step(self, action, step):
        ...
        if self.state_space == "image" and self.action_space != "continuous":
            return StepFollowLane.step_followlane_state_image_actions_discretes(
                self, action, step
            )
        elif self.state_space == "image" and self.action_space == "continuous":
            return StepFollowLane.step_followlane_state_image_actions_continuous(
                self, action, step
            )
        elif self.state_space != "image" and self.action_space == "continuous":
            return StepFollowLane.step_followlane_state_sp_actions_continuous(
                self, action, step
            )
        else:
