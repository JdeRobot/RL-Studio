...
class TrainerFollowLanePPOCarla:
    ...
    def one_step_iteration(self, episode, step, prev_state, cumulated_reward):
        ...
        state, reward, done, info = self.env.step([action], step)
        # fps = info["fps"]
        ...
        if self.global_params.show_monitoring:
            ...
            return state, cumulated_reward, loss
    ...
    def main(self):
        ...
        for episode in tqdm(
            ...
            while not done:
                state, cumulated_reward, loss = self.one_step_iteration(episode, step, prev_state, cumulated_reward)
                prev_state = state
                step += 1
            ...
            self.send_and_store_metrics(episode, loss, start_time_epoch)
