...
class StatsDataFrame:
    def save_dataframe_stats(
        self, environment, outdir, stats, q_table=False, sac=False
    ):
        ...
        if q_table:
            ...
            file_excel = f"{outdir}/{time.strftime('%Y%m%d-%H%M%S')}_QTABLE_Circuit-{environment['town']}_States-{environment['states']}_Actions-{environment['action_space']}_Rewards-{environment['reward_function']}.xlsx"
        elif sac:
            file_csv = f"{outdir}/{time.strftime('%Y%m%d-%H%M%S')}_SAC_Circuit-{environment['town']}_States-{environment['states']}_Actions-{environment['action_space']}_Rewards-{environment['reward_function']}.csv"
            file_excel = f"{outdir}/{time.strftime('%Y%m%d-%H%M%S')}_SAC_Circuit-{environment['town']}_States-{environment['states']}_Actions-{environment['action_space']}_Rewards-{environment['reward_function']}.xlsx"
        else:
