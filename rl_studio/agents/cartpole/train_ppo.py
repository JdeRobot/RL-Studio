...
from rl_studio.algorithms.ppo import Actor, Critic, Mish, t
    ...
    def main(self):
        ...
        for run in tqdm(range(self.RUNS)):
            ...
            while not done:
                ...
                dist = self.actor.forward(t(state))
                action = dist.sample()
                ...
                if prev_prob_act:
                    actor_loss = self.actor.train(prev_prob_act, prob_act, advantage, global_steps);
                    critic_loss = self.critic.train(advantage, global_steps)
                    prev_prob_act = prob_act

                # w.add_scalar("reward/episode_reward", episode_rew, global_step=i)
